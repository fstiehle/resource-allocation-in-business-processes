...
data <- read_csv("Projects/RA_SLR/Literature_review_collection.xlsx - DataExtraction.csv", 
...
ggplot(data, aes(x=Year)) +
  ...
  xlab("Number of Publications") + 
  scale_x_date(date_labels = "%Y", breaks = "2 years", minor_breaks = NULL) +
...
ggplot(mat, aes(y=`Evaluation Type`, fill=`Prototypical implementation type`)) +
  geom_bar() +
  scale_fill_manual(name = element_blank(), values = c("available"= colors_rating[5],
